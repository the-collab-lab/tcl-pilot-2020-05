...
const PinDescription = ({title,image,description}) => {
  ...
  return (
     <div className = "Description">
            ...
            {image? (<img src={image} alt={firstCap(title)} />):(<img src={DEFAULT_IMAGE} alt={firstCap(title)} />)}
            {description? <p>Description: {firstCap(description)}</p>:<p>No Description</p>}
            <p><a target="_blank" rel="noopener noreferrer" href={ wikipedia }>find out more about this spot over on Wikipedia</a></p>
     </div>
   )
}
